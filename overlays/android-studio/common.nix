...
{ alsa-lib, bash, buildFHSUserEnv, cacert, coreutils, dbus, expat, fetchurl
, findutils, file, fontsConf, git, glxinfo, gnugrep, gnused, gnutar, gtk2
, gnome_vfs, glib, GConf, gzip, fontconfig, freetype, libpulseaudio, libGL
...
, which, runCommand, xkeyboard_config, zlib, makeDesktopItem }:

let
  drvName = "android-studio-${channel}-${version}";
  filename = "android-studio-"
    + (if (build != null) then "ide-${build}" else version) + "-linux.tar.gz";
